...
_Learning from networks_ course (TODO COURSE ID) at DEI UniPD, academic year 2023-2024, personal contribution of student ID: 2019122
...
Most of the python scripts are configured entirely through `toml` files that can usually be stored on a separate location. All models have been tuned multiple times with the same parameters but according to different random seed. Random seeds are stored in file `computed/seedsSequence_p10.txt` . Modules installed on the virtual environment of the project were *pip-freezed* in `DEPENDENCIES.txt`
...
Three random graph generators were developed in this work. Among these, only one was used to successfully generate 100 complete random collections (each counting 756 graphs)

**Chung-Lu sampling based on observed node labels adjacency**. TODO

![]()

**Random Walk generator driven node label adjacency**. TODO

TODO  hyperparameters walk lenght and number of walks

**Soft edge swapping controlled by CL probabilities and node label adjacency**. TODO
...
<img title="" src="img/DHFR_sample.png" alt="" width="350" data-align="inline">
...
![](/home/leonardo/Pubblici/shareWifi/LFN/img/statisticalSignificance_240119-172925.png)
...
Multi-Layer Perceptron (MLP) is a very popular machine learning model. Here we consider a simplified MLP architecture having only one hidden layer, ReLU activation functions, and dropout regularisation.

Torch classes [Linear](https://pytorch.org/docs/stable/generated/torch.nn.Linear.html), [ReLU](https://pytorch.org/docs/stable/generated/torch.nn.ReLU.html#relu), [Softmax](https://pytorch.org/docs/stable/generated/torch.nn.Softmax.html#softmax)  are used in a 2 layer model in script `traditionalMLP.py` which is configured by `traditionalMLP.toml` where the hyper-parameters of the model are exposed :
...
First decide which optimiser is best, then find the optimal dimension of the hidden layer
...
**Warning.** Modify PyG `node2vec.py` lines XX. Not clear if this is a issue, but the behaviour has been reported to the official PyG developers as issue YYY
...
![](img/embedMLP_240122-162854.png)

explain first figure, introduce second

![]()

Model XXX is therefore retained as best performance indicator of the architecture DeepWalk + MLP and compared against other models in [Sec.7](#sec7), training log and output predictions are stored as **XXXX** in `computed/embedingMLP/`.
...
- [ ] A modified dataset DHFR-MD is available at [networkrepository.com](https://networkrepository.com/DHFR-MD.php) [[4]](#4) but has not been considered in the current stage of this work.
...
- [ ] Compute more node features by counting loops of arbitrary length in each molecule. Make use of graph-tool library

- [ ] Explore other aggregation functions for the message passing of each convolution layer and more global node aggregation layers for representing the graph before the prediction
...
- [ ] Consider more random seed on the tuned models in order to better approximate the noise of the random shuffling

- [ ] improve the classification performance plots by displaying the score of training and validation for each seed. This is complicated because for each seed you need to append the information in the log file generated together with the posterior probabilities or the log file
